...
'''
    ...
    def _generate_applications(self, network: NetworkModel, sim_config: SimulationConfig) -> str:
        ...
        lines = [
            ...
            "    # ============================================",
            "",
            "    # List to collect receiver poll functions for step-based simulation",
    ...
    def _generate_custom_app_flow(
        ...
        # Check if target has a custom app script for receiving
        target_has_script = target_node and target_node.has_app_script
        ...
        # Setup receiver - use custom receiver if target has script
        if target_has_script:
            # Generate receiver that uses target node's app script
            target_class_name = self._sanitize_class_name(target_node.name) + "App"
            target_module_name = self._sanitize_module_name(target_node.name)
            recv_var = f"recv_app_{flow_idx}"
            
            lines.extend([
                f"    # ------------------------------------------------",
                f"    # Receiver using {target_node.name}'s custom app script",
                f"    # ------------------------------------------------",
                f"    {recv_var} = None",
                f"    {recv_var}_socket = None",
                f"    {recv_var}_count = [0]",
                "",
                f"    # Load target's custom application for receive callbacks",
                f"    try:",
                f"        from {target_module_name} import {target_class_name}",
                f"        {recv_var}_config = {{",
                f"            'node': nodes.Get({target_idx}),",
                f"            'target_address': '',",
                f"            'target_port': {port},",
                f"            'protocol': '{protocol}',",
                f"            'start_time': {flow.start_time},",
                ...
                f"            'send_interval': {send_interval},",
                f"            'packet_size': {flow.packet_size or 512},",
                f"            'app_name': '{target_node.name}',",
                f"            'source_node_name': '{target_name}',",
                f"            'target_node_name': '{source_name}',",
                f"        }}",
                f"        {recv_var} = {target_class_name}({recv_var}_config)",
                f"        {recv_var}.setup()",
                f"        print(f'[Setup] Custom receiver {target_node.name} ready on {target_name}')",
                f"    except ImportError:",
                f"        # Module not available as separate file - will use embedded script",
                f"        print(f'[Info] Will use embedded receiver for {target_node.name}')",
                f"        {recv_var} = None",
                f"    except Exception as e:",
                f"        print(f'[Warning] Could not setup receiver app: {{e}}')",
                f"        {recv_var} = None",
                "",
                f"    # Create receiver socket",
                f"    {recv_var}_socket = ns.Socket.CreateSocket(",
                f"        nodes.Get({target_idx}),",
                f"        ns.{protocol.capitalize()}SocketFactory.GetTypeId()",
                f"    )",
                f"    recv_local_{flow_idx} = ns.InetSocketAddress(ns.Ipv4Address.GetAny(), {port})",
                f"    {recv_var}_socket.Bind(recv_local_{flow_idx}.ConvertTo())",
                f"    print(f'[0.000s] [{target_name}] Receiver socket bound to port {port}')",
                "",
                f"    def poll_receiver_{flow_idx}():",
                f"        '''Poll for received packets - called from simulation loop.'''",
                f"        if {recv_var}_socket is None:",
                f"            return",
                "",
                f"        while True:",
                f"            try:",
                f"                packet = {recv_var}_socket.Recv(65535, 0)",
                f"                # Check if we got a valid packet",
                f"                if packet is None:",
                f"                    break",
                ...
                f"                    break",
                "",
                f"                {recv_var}_count[0] += 1",
                f"                seq = {recv_var}_count[0]",
                f"                current_t = ns.Simulator.Now().GetSeconds()",
                "",
                f"                # Extract payload using ctypes buffer (ns-3.45 compatible)",
                f"                import ctypes",
                f"                buf_type = ctypes.c_uint8 * size",
                f"                buf = buf_type()",
                f"                packet.CopyData(buf, size)",
                f"                payload = bytes(buf)",
                "",
                f"                # Call custom on_packet_received if available",
                f"                if {recv_var} and hasattr({recv_var}, 'on_packet_received'):",
                f"                    try:",
                f"                        {recv_var}.on_packet_received(seq, payload)",
                f"                    except Exception as e:",
                f"                        print(f'[{{current_t:.3f}}s] [{target_name}] on_packet_received error: {{e}}')",
                f"                else:",
                f"                    # Default logging",
                f"                    print(f'[{{current_t:.3f}}s] [{target_name}] Received packet #{{seq}}: {{size}} bytes')",
                f"                    try:",
                f"                        print(f'[{{current_t:.3f}}s] [{target_name}]   Content: {{payload.decode(\"utf-8\")}}')",
                f"                    except:",
                f"                        print(f'[{{current_t:.3f}}s] [{target_name}]   Hex: {{payload.hex()}}')",
                f"            except Exception as e:",
                f"                # No more packets or error",
                f"                if 'null-pointer' not in str(e):",
                f"                    print(f'[{{ns.Simulator.Now().GetSeconds():.3f}}s] [{target_name}] Recv error: {{e}}')",
                f"                break",
                "",
                f"    # Register poll function for step-based simulation",
                f"    _receiver_poll_functions.append(poll_receiver_{flow_idx})",
                "",
            ])
        else:
            # Use standard PacketSink for target without script
            lines.extend([
    ...
    def _generate_custom_script_receiver(
        ...
        lines = [
            ...
            f"        print(f'[{{ns.Simulator.Now().GetSeconds():.3f}}s] [{app_name}] Receiver listening on port {port}')",
            f"        # Start polling for received packets",
            f"        ns.Simulator.Schedule(ns.Seconds(0.01), poll_recv_{app_idx})",
            "",
            f"    def poll_recv_{app_idx}():",
            f"        '''Poll for received packets and call on_packet_received.'''",
            f"        if recv_socket_{app_idx} is None:",
            f"            return",
            "",
            f"        # Check for received data (non-blocking)",
            f"        while True:",
            f"            packet = recv_socket_{app_idx}.Recv(65535, 0)",
            f"            if packet is None or packet.GetSize() == 0:",
            f"                break",
            "",
            f"            recv_count_{app_idx}[0] += 1",
            f"            seq = recv_count_{app_idx}[0]",
            "",
            f"            # Extract payload from packet",
            f"            size = packet.GetSize()",
            f"            buf = bytes(size)",
            f"            packet.CopyData(buf, size)",
            "",
            f"            # Call custom on_packet_received if defined",
            f"            if hasattr({safe_name}_module, 'on_packet_received'):",
            f"                try:",
            f"                    {safe_name}_module.on_packet_received(seq, buf)",
            f"                except Exception as e:",
            f"                    print(f'[{{ns.Simulator.Now().GetSeconds():.3f}}s] [{app_name}] on_packet_received error: {{e}}')",
            f"            else:",
            f"                # Default behavior: log received packet",
            f"                print(f'[{{ns.Simulator.Now().GetSeconds():.3f}}s] [{app_name}] Received packet #{{seq}}: {{size}} bytes')",
            f"                try:",
            f"                    text = buf.decode('utf-8')",
            f"                    print(f'[{{ns.Simulator.Now().GetSeconds():.3f}}s] [{app_name}]   Content: {{text}}')",
            f"                except:",
            f"                    print(f'[{{ns.Simulator.Now().GetSeconds():.3f}}s] [{app_name}]   Hex: {{buf.hex()}}')",
            "",
    ...
    def _generate_default_receiver(
        ...
        return [
            ...
            f"            buf = bytes(size)",
            f"            packet.CopyData(buf, size)",
            f"            print(f'[{{ns.Simulator.Now().GetSeconds():.3f}}s] [{app_name}] Received packet #{{recv_count_{app_idx}[0]}}: {{size}} bytes')",
            f"            try:",
            f"                print(f'[{{ns.Simulator.Now().GetSeconds():.3f}}s] [{app_name}]   Content: {{buf.decode(\"utf-8\")}}')",
            f"            except:",
            f"                print(f'[{{ns.Simulator.Now().GetSeconds():.3f}}s] [{app_name}]   Hex: {{buf.hex()}}')",
            f"        ns.Simulator.Schedule(ns.Seconds(0.01), poll_recv_{app_idx})",
            "",
    ...
    def _generate_simulation_run(self, sim_config: SimulationConfig, output_dir: str) -> str:
        ...
        lines = [
            ...
            "",
            "        # Build list of all event times for all apps",
            "        # Events: start, send (multiple), stop",
            f"        end_time = {sim_config.duration}",
            "        precision = 0.001  # 1ms precision for time comparisons",
            "",
            "        # Collect all scheduled event times",
            "        event_times = set()",
            "        app_send_times = {}  # app_index -> list of send times",
            "",
            "        for i, app in enumerate(custom_apps):",
            "            # Add start time",
            "            event_times.add(app.start_time)",
            "            # Add stop time",
            "            event_times.add(app.stop_time)",
            "",
            "            # Calculate all send times for this app",
            "            send_times = []",
            "            t = app.start_time",
            "            while t < app.stop_time and t < end_time:",
            "                send_times.append(round(t, 3))  # Round to ms precision",
            "                event_times.add(round(t, 3))",
            "                t += app.send_interval",
            "            app_send_times[i] = send_times",
            "",
            "        # Add simulation end time",
            "        event_times.add(end_time)",
            "",
            "        # Sort event times",
            "        event_times = sorted(event_times)",
            "        print(f'Scheduled {len(event_times)} event times')",
            "",
            "        # Track which sends have been done",
            "        app_send_index = {i: 0 for i in range(len(custom_apps))}",
            "",
            ...
            "                break",
            "",
            "            # Advance simulation to this event time",
            "            ns.Simulator.Stop(ns.Seconds(event_time))",
            "            ns.Simulator.Run()",
            "            current_time = ns.Simulator.Now().GetSeconds()",
            "",
            "            # Poll receivers for any incoming packets",
            "            for poll_func in _receiver_poll_functions:",
            "                try:",
            "                    poll_func()",
            "                except Exception as e:",
            "                    print(f'[{current_time:.3f}s] Receiver poll error: {e}')",
            "",
            "            # Process events for each app at this time",
            "            for i, app in enumerate(custom_apps):",
            "                # Check for start event",
            "                if not app.is_running and abs(current_time - app.start_time) < precision:",
            "                    app.start()",
            "",
            "                # Check for send events",
            "                if app.is_running and current_time < app.stop_time:",
            "                    send_times = app_send_times[i]",
            "                    send_idx = app_send_index[i]",
            "                    while send_idx < len(send_times) and send_times[send_idx] <= current_time + precision:",
            "                        app.send_packet()",
            "                        send_idx += 1",
            "                    app_send_index[i] = send_idx",
            "",
            "                # Check for stop event",
            "                if app.is_running and abs(current_time - app.stop_time) < precision:",
            "                    app.stop()",
            "",
            "        # Final poll for any remaining received packets",
            "        for poll_func in _receiver_poll_functions:",
            "            try:",
            "                poll_func()",
            "            except:",
            "                pass",
            "",
            "        # Final cleanup - stop any still-running apps",
            "        for app in custom_apps:",
            "            if app.is_running:",
            "                app.stop()",
            "    else:",
            "        # No custom apps - check if we have receiver poll functions",
            "        if _receiver_poll_functions:",
            "            # Run step-by-step to poll receivers",
            "            step_interval = 0.01  # Poll every 10ms",
            "            current = 0.0",
            f"            while current < {sim_config.duration}:",
            "                ns.Simulator.Stop(ns.Seconds(min(current + step_interval, end_time)))",
            "                ns.Simulator.Run()",
            "                current = ns.Simulator.Now().GetSeconds()",
            "                for poll_func in _receiver_poll_functions:",
            "                    try:",
            "                        poll_func()",
            "                    except:",
            "                        pass",
            "        else:",
            "            # No custom apps or receivers, run normally",
            "            ns.Simulator.Run()",
            "",
        ]
...
'''
    ...
    def get_required_files(
        ...
        # Export ALL nodes that have app scripts attached
        # This ensures both senders and receivers have their scripts available
        for node_id, node in network.nodes.items():
            if node.has_app_script and node.id not in added_nodes:
                module_name = self._sanitize_module_name(node.name)
                files.append({
                    'type': 'custom',
                    'source_path': None,
                    'dest_name': f'{module_name}.py',
                    'content': node.app_script,
                })
